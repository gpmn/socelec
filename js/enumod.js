...
function loadMatches(enuAddr){
    var options = {
        httpEndpoint   : enuAddr,
        expireInSeconds: 60,
        broadcast      : true,
        debug          : false,
        sign           : false
...
function loadGames(enuAddr, matchid){
    var options = {
        httpEndpoint   : enuAddr,
        expireInSeconds: 60,
        broadcast      : true,
...
function loadUserBet(enuAddr, matchid, account){
    var options = {
        httpEndpoint   : enuAddr,
        expireInSeconds: 60,
        broadcast      : true,
    ...
    for(var index = 1; index <= 11; index ++){
        ...
        var pm = enu.getTableRows({
            code: SOCCER_ACCOUNT,
            scope: Enu.modules.format.decodeName(gameid, false),
            table: "betinfo",
            limit: 1,
            lower_bound: Enu.modules.format.encodeName(account, false),
            json: true
        });
        allPm.push(pm);
    }
...
function getCredit(enuAddr, account){
    var options = {
        httpEndpoint   : enuAddr,
        expireInSeconds: 60,
        broadcast      : true,
...
function submitBet(enuAddr, account, activeKey, gameid, money, side, lTeam, rTeam, concede){
    var options = {
        httpEndpoint   : enuAddr,
...
function deposit(enuAddr, account, activeKey, money){
    var options = {
        httpEndpoint   : enuAddr,
        keyProvider    : [activeKey],
        expireInSeconds: 60,
...
function applyWithdraw(enuAddr, account, activeKey, money){
    var options = {
        httpEndpoint   : enuAddr,
        keyProvider    : activeKey,
        expireInSeconds: 60,
...
function cancelWithdraw(enuAddr, account, activeKey){
    var options = {
        httpEndpoint   : enuAddr,
        keyProvider    : activeKey,
        expireInSeconds: 60,
